...
class TenBlock():
    ...
    def nearCanableBlock(self,x,y):
        nearblocked = []
        for a,b in yeiAcro(x,y):
            c = self.getBlockfiller(a,b)
            if(c>=3):
                nearblocked.append((x,y))
        for a,b in yeihorc(x,y):
            c = self.getBlockfiller(a,b)
            if(c>=3):
                nearblocked.append((x,y))
        return nearblocked
    ...
    def nearCanableBlockBlocked(self,x,y):
        nearblockblocked = []
        for a,b in yeihorc(x,y):
            temp = self.nearCanableBlock(a,b)
            if(len(temp)!= 0):
                for t,s in temp:
                    temp2 = list(filter(lambda c :  c[0] == t and c[1] == s, nearblockblocked))
                    if(len(temp2) == 0):
                        nearblockblocked.append((t,s))
        ...
        # print(nearblockblocked)           
        return len(nearblockblocked)
...
def horzcalla(x,y ,ox, oy,bx,by):
    ...
    if(point[1] == maxPoint and ( ox != x+1 and y != oy and bx != x+1 and by != y) and not mainBlock.inroundzero(x+1, y) and not mainBlock.getBlockfiller(x+1, y)==3):
        return x+1, y , point[1]
    if(point[2] == maxPoint and ( ox != x and y-1 != oy and bx != x and by != y-1 ) and not mainBlock.inroundzero(x, y-1) and not mainBlock.getBlockfiller(x, y-1)==3):
        return x, y-1 , point[2]
    if(point[3] == maxPoint and ( ox != x and y+1 != oy and bx != x and by != y+1) and not mainBlock.inroundzero(x, y+1) and not mainBlock.getBlockfiller(x, y+1)==3):
        return x, y+1 , point[3]
    if(point[4] == maxPoint and ( ox != x-1 and y != oy and bx != x-1 and by != y) and not mainBlock.inroundzero(x-1, y) and not mainBlock.getBlockfiller(x-1, y)==3):    
...
def checkPoint(x, y, ox, oy, bx, by, x2,y2): #우선순위 탐색
    ...
    #     point -= 4
    # print(mainBlock.nearCanableBlockBlocked(x,y))
    ...
    if(testCopy.horzisGetWorse(x,y)):
        point -= 3
    point -= (testCopy.del_count)/2
    point -= (mainBlock.nearCanableBlockBlocked(x,y)) # 빈 그릇이 block 일 시 우선순위 하향
    c = LogB.count((x,y))
    if(c != 0 ):
        n = LogB[::-1].index((x,y))
        point -= c + (n / len(LogB))
    ...
    point += ((mainBlock.mostRoundZero(x,y)) - testCopy.mostRoundZero(x,y))/4 #근처에 0이 많으면 상승 수정 후 0이 많아지면 감소
